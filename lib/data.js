...
export const getEvents = async (timeoutMs = null) => {
  ...
  try {
    const response = await fetchWithTimeout(
      'https://api.kommunity.com/api/v1/happyhackingspace/events',
      {},
      timeoutMs
    )
    ...
    // This prevents build failures when the API is temporarily unavailable
    console.warn('Returning empty events array due to API failure')
    return []
  }
}
